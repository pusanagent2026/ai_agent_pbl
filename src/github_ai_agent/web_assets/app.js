...
async function loadConfig() {
  ...
  if (!selectedRepository.owner || !selectedRepository.repo) {
    ...
    answer.innerHTML =
      "<span class='error'>GitHub \uC5F0\uACB0 \uD6C4 \uC800\uC7A5\uC18C\uB97C \uC120\uD0DD\uD574\uC57C \uBD84\uC11D\uC744 \uC2DC\uC791\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4.</span>";
  }
  ...
  setServiceCard(
    connectGithub,
    selectedRepoLabel || "GitHub 연결",
    Boolean(selectedRepoLabel || githubConnected),
  );
  setServiceCard(connectNotion, "Notion 연결", notionEnabled);
  setServiceCard(
    connectGoogle,
    connectedGoogleUser
      ? `Calendar: ${connectedGoogleUser}`
      : "Google Calendar 연결",
    calendarEnabled,
  );
  const devBypassConnections =
  ...
  googleCalendarLink.href = calendarEnabled
    ? googleCalendarUrl(connectedGoogleUser)
    : "/auth/google";
  await loadCalendarEvents();
  renderMembers(config.members || [], config.member_warnings || []);
  if (
    selectedRepository.owner !== config.owner ||
    selectedRepository.repo !== config.repo
  ) {
...
async function loadCalendarEvents() {
  if (!calendarEnabled) {
    calendarEvents.innerHTML =
      "<span class='chip'>Google Calendar 연결 필요</span>";
    return;
  }
...
function renderNotionDatabases(databases, selectedDatabaseId, notionConnected) {
  ...
  if (!databases.length) {
    ...
    option.value = "";
    option.textContent = notionConnected
      ? "Notion 연결됨 - 선택 가능한 DB 없음"
      : "Notion 연결 필요";
...
function renderCalendarMonth(events) {
  ...
  if (!events.length) {
    calendarEvents.innerHTML =
      "<span class='chip'>달력에 표시할 일정 없음</span>";
    return;
  }
...
function readSavedRepository(config) {
  ...
  if (saved) {
    try {
      ...
      const stillAvailable = repositories.some(
        (repository) =>
      ...
      if (
        parsed.owner &&
        parsed.repo &&
        (!repositories.length || stillAvailable)
      ) {
        return parsed;
      }
    } catch (_) {}
  ...
  if (config.owner && config.repo) {
    const matched = repositories.find(
      (repository) =>
        repository.owner === config.owner && repository.repo === config.repo,
    );
    const fallback = {
      owner: config.owner,
      repo: config.repo,
      installation_id: matched
...
function renderRepositorySelect(repositories) {
  ...
  repositories.forEach((repository) => {
    ...
    option.selected =
      repository.owner === selectedRepository.owner &&
      repository.repo === selectedRepository.repo;
...
async function loadBranches() {
  if (!selectedRepository.owner || !selectedRepository.repo) {
    branchSelect.innerHTML =
      "<option value=''>먼저 저장소를 연결하세요</option>";
    return;
  }
  ...
  try {
    ...
    branchSelect.innerHTML =
      "<option value=''>브랜치를 불러오지 못했습니다</option>";
...
async function loadFileTree(branch) {
  ...
  fileListEl.innerHTML =
    "<span class='muted'>파일 목록을 불러오는 중...</span>";
  try {
    const params = new URLSearchParams({
      owner: selectedRepository.owner,
      repo: selectedRepository.repo,
...
function renderFileList(truncated) {
  ...
  filtered.forEach((file) => {
    ...
    div.className =
      "file-item" + (file.path === selectedFilePath ? " selected" : "");
...
function renderReviewErrors(errors) {
  if (!errors || !errors.length) {
    reviewErrors.innerHTML =
      "<span class='muted'>발견된 오류가 없습니다.</span>";
    return;
  }
  reviewErrors.innerHTML = "";
  errors.forEach((item) => {
...
async function reviewSelectedFile() {
  ...
  try {
    ...
    reviewComments.innerHTML =
      "<span class='muted'>리뷰에 실패했습니다.</span>";
...
function refreshApproveButtons() {
  ...
  updateNotionSaveModeFromText(
    lastUserQuestion || question.value || answer.textContent,
  );
...
function renderMembers(members, warnings) {
  ...
  if (!members.length) {
    ...
    members.forEach((member) => {
      ...
      span.className = "chip";
      const label =
        member.github_id || member.login || member.name || "unknown";
      span.title =
        member.role || member.contributions
          ? `${member.role || ""} ${member.contributions || ""}`.trim()
          : "";
...
function inferNotionSaveMode(text) {
  ...
  if (
    /(체크리스트|체크\s*리스트|checklist|check list|체크박스|todo\s*list)/i.test(
      normalized,
    )
  ) {
    return "checklist";
  ...
  if (
    /(데이터베이스|database|\bdb\b|표|테이블|행으로|팀원별|담당자별|할\s*일|작업\s*분배|업무\s*분배|db\s*생성|db\s*만들|데이터베이스\s*생성|데이터베이스\s*만들)/i.test(
      normalized,
    )
  ) {
    return "database";
  ...
  if (
    /(페이지|문서|기록|정리|분석\s*내용|코드\s*리뷰|리뷰\s*내용|본문|노트)/i.test(
      normalized,
    )
  ) {
    return "page";
...
function updateNotionSaveModeFromText(text) {
  ...
  shouldCreateNotionDatabaseBeforeSave =
    /(db|데이터베이스).{0,12}(생성|만들|새로|자동)|(?:생성|만들|새로|자동).{0,12}(db|데이터베이스)/i.test(
      text || "",
    );
  if (notionSaveMode) {
    notionSaveMode.value = currentNotionSaveMode;
  }
  approveNotion.textContent =
    shouldCreateNotionDatabaseBeforeSave && currentNotionSaveMode === "database"
...
function canApproveNotion() {
  ...
  const hasAnswer = Boolean(
    answer && answer.textContent && answer.textContent.trim(),
  );
  return (
    notionEnabled &&
    (hasTasks ||
      (hasAnswer && ["page", "checklist"].includes(currentNotionSaveMode)))
  );
}
...
function isFollowUpNotionSaveRequest(text) {
  ...
  const refersPrevious =
    /(방금|위\s*내용|이\s*내용|이전|아까|분석\s*내용|답변|결과)/i.test(
      normalized,
    );
  return (
    mentionsNotion &&
    asksToSave &&
    refersPrevious &&
    Boolean(lastAnalysisAnswerText || answer.textContent.trim())
  );
}
...
async function savePreviousAnswerToNotion(text) {
  ...
  const currentAnswer = appendAssistantChatMessage(
    `방금 분석 내용을 '${notionSaveModeLabel(saveMode)}' 방식으로 Notion에 저장하는 중입니다.`,
  );
...
async function analyzeGithub() {
  ...
  if (needsGithub && (!selectedRepository.owner || !selectedRepository.repo)) {
    ...
    renderTaskEmptyState(
      "저장소가 선택되면 GitHub 기록을 분석할 수 있습니다.",
      "GH",
    );
    connectGithub.focus();
    return;
...
async function postSelectedTasks(url, savingText, successText) {
  ...
  const saveMode = updateNotionSaveModeFromText(
    lastUserQuestion || question.value || answer.textContent,
  );
  if (
    !items.length &&
    (!isNotionRequest || !["page", "checklist"].includes(saveMode))
  ) {
  ...
  try {
    ...
    const links = (payload.created || [])
      ...
      .map(
        (item, index) =>
          `<a href="${escapeHtml(item.url)}" target="_blank" rel="noreferrer">Notion에서 열기 ${index + 1}</a>`,
      )
      .join("<br>");
...
approveNotion.addEventListener("click", () =>
  postSelectedTasks(
    "/api/approve-tasks",
    "Saving to Notion...",
    "Notion 등록 완료",
...
approveCalendar.addEventListener("click", () =>
  postSelectedTasks(
    "/api/approve-calendar-events",
    "Saving to Calendar...",
    "Calendar 등록 완료",
...
async function loadReadmeBranches() {
  ...
  readmeBranchSelect.innerHTML =
    "<option value=''>브랜치를 불러오는 중...</option>";
  try {
    const params = new URLSearchParams({
      owner: selectedRepository.owner,
      repo: selectedRepository.repo,
...
function renderReadmeDiff(diff, fallbackText) {
  ...
  readmeDiff.innerHTML = rows
    .map((row) => {
      const leftCls =
        row.left_type === "remove"
          ? "diff-remove"
          : row.left_type === "empty"
            ? "diff-empty"
            : "";
...
function applyReadmeAnalysisResult(branch, payload) {
  ...
  if (!payload.relevant) {
    ...
    readmeVerdict.textContent =
      "관련 변경이지만 재작성 결과가 기존 README와 동일합니다.";
    readmeProposal = null;
    applyReadmeUpdateButton.disabled = true;
  } else {
...
function loadBranchShaBaseline() {
  try {
    branchShaBaseline = JSON.parse(
      localStorage.getItem(baselineStorageKey()) || "{}",
    );
  } catch (_) {
    branchShaBaseline = {};
  }
